...
日本各地に伝わる民間伝承を、現代のWeb技術およびGIS（地理情報システム）を用いて可視化・構造化する試みです。

単なる「怖い話マップ」ではなく、土地の歴史的・文化的文脈を含めて整理・参照できる、包括的な情報管理プラットフォームの構築を目的としています。
...
- **Google Mapsと古地図の比較**: Google Mapsと明治時代の古地図の重ね合わせの表示
...
### 投稿
- **投稿の順序**:投稿したからと言ってすぐに公開されるわけではなく 下書き → 運営によるチェック → 公開　の順
- **出典必須**: すべての伝承に信頼できる出典情報（URL/書籍/インタビュー）を要求

### 通報・モデレーション
- **通報機能**: 不適切なコンテンツ、誤情報、差別、プライバシー侵害をユーザーが通報できる
- **通報管理**: レビュワーが通報を確認・処理
